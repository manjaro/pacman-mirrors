#!/usr/bin/env python3
"""Manjaro-Mirrors Configuration"""
ENV = "dev"
# http constants
URL_MIRROR_JSON = "http://repo.manjaro.org/mirrors.json"
URL_STATUS_JSON = "http://repo.manjaro.org/status.json"

if ENV == "production":
    # dir constant
    MIRROR_DIR = "/var/lib/manjaro-mirrors/"
    # file constant
    CUSTOM_FILE = MIRROR_DIR + "custom-mirrors.json"
    CONFIG_FILE = "/etc/pacman-mirrors.conf"
    HTTP_FALLBACK = MIRROR_DIR + "mirrors.json"
    MIRROR_FILE = "/usr/lib/manjaro-mirrors/mirrors.json"
    MIRROR_LIST = "/etc/pacman.d/mirrorlist"
    STATUS_FILE = MIRROR_DIR + "status.json"
    # special case
    O_CUST_FILE = "/var/lib/pacman-mirrors/Custom"
else:
    # dir constant
    MIRROR_DIR = "dev/var/lib/manjaro-mirrors/"
    # file constant
    CUSTOM_FILE = MIRROR_DIR + "custom-mirrors.json"
    CONFIG_FILE = "dev/etc/pacman-mirrors.conf"
    HTTP_FALLBACK = MIRROR_DIR + "mirrors.json"
    MIRROR_FILE = "dev/usr/lib/manjaro-mirrors/mirrors.json"
    MIRROR_LIST = "dev/etc/pacman.d/mirrorlist"
    STATUS_FILE = MIRROR_DIR + "status.json"
    # special case
    O_CUST_FILE = "dev/var/lib/pacman-mirrors/Custom"
# repo constants
BRANCHES = ("stable", "testing", "unstable")
REPO_ARCH = "$repo/$arch"
